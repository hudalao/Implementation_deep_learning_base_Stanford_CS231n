...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        # calculation of loss
        h0, cache_h0 = affine_forward(features, W_proj, b_proj)  # h0 in the graph
        x, cache_x = word_embedding_forward(captions_in, W_embed) # input x for hidden states
        ...
        dW_proj, db_proj = affine_backward(dh0, cache_h0)
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        captions = []
        x0 = np.repeat(self._start, N)
        captions.append(x0)
        h, _ = affine_forward(features, W_proj, b_proj)  # h0 in the graph
        x, _ = word_embedding_forward(self._start, W_embed) # input x for hidden states
        while x != self._end:
            h, _ = rnn_step_forward(x, h, Wx, Wh, b)
            out, _ = temporal_affine_forward(h, W_vocab, b_vocab)
